    ...
    public Object execute(TransitionSystem ts, Unifier un, Term[] args) throws Exception {
        ...
        if (function == 1) {
            ...
            for (int i = 0; i < map.size(); i++) {
                ...
                String details = entry[2];
                String type = entry[3];

                String[] newentry = {Integer.toString(x), Integer.toString(y), entry[2], entry[3]};
                map.set(i, newentry);
            }
            ...
            for (int i = 0; i < map.size(); i++) {
                String[] entry2 = map.get(i);
                if(entry[0].equals(entry2[0]) && entry[1].equals(entry2[1]) && entry[2].equals(entry2[2]) && entry[3].equals(entry2[3])) {
                    // Nothing
                } else {
                    add = true;
                }
